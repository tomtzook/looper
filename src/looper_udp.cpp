...
udp create_udp(const loop loop) {
    ...
    auto [handle, udp_impl] = data.m_udps.allocate_new(data.m_context.get());
    looper_trace_info(log_module, "created new udp: loop=%lu, handle=%lu", data.m_handle, handle);
    data.m_udps.assign(handle, std::move(udp_impl));
...
void destroy_udp(const udp udp) {
    ...
    const auto udp_impl = data.m_udps.release(udp);
    udp_impl->close();
}
...
void write_udp(const udp udp, inet_address_view destination, const std::span<const uint8_t> buffer, udp_callback&& callback) {
    ...
    looper_trace_info(log_module, "writing to udp: loop=%lu, handle=%lu, data_size=%lu, to=%s:%d", data.m_handle, udp, buffer.size_bytes(), destination.ip.data(), destination.port);
