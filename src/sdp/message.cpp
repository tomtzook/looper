...
static std::unordered_map<std::string, std::unique_ptr<_base_attribute_holder_creator>>& _get_holders() {
...
std::optional<std::unique_ptr<_base_attribute_holder>> _create_attribute(const std::string& name) {
    const auto it = _get_holders().find(name);
    if (it != _get_holders().end()) {
        return it->second->create();
    } else {
...
std::istream& message::operator>>(std::istream& is) {
    while (is.peek() != is.eof()) {
        ...
        if (name == "a") {
            ...
            const auto attr_name = serialization::read_until(is, ':');
            serialization::consume(is, ':');
            serialization::consume_whitespaces(is);

            auto holder_opt = attributes::_create_attribute(attr_name);
            if (holder_opt) {
                auto holder = std::move(holder_opt.value());
                holder->read(is);
                add_attribute(name, std::move(holder));
            } else {
                auto holder = std::make_unique<attributes::_attribute_holder<attributes::generic_attribute>>();
                holder->value.name = attr_name;
                holder->read(is);
                add_attribute(name, std::move(holder));
            }
        } else {
...
std::ostream& message::operator<<(std::ostream& os) {
    ...
    // dump attributes
    for (auto& [name, holders] : m_attributes) {
        for (const auto& holder : holders) {
            os << "a=" << name << ":";
            holder->write(os);
            os << "\r\n";
...
void message::add_attribute(const std::string& name, std::unique_ptr<attributes::_base_attribute_holder> holder) {
    const auto it = m_attributes.find(name);
    if (it == m_attributes.end()) {
        std::vector<std::unique_ptr<attributes::_base_attribute_holder>> vector;
        vector.push_back(std::move(holder));
        m_attributes[name] = std::move(vector);
    } else {
        it->second.push_back(std::move(holder));
    }
}
